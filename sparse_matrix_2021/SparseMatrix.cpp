...
ProxyOne SparseMatrix::operator+ (const size_t index) {
...
ProxyOneConst SparseMatrix::operator+ (const size_t index) const{
...
ProxyTwo SparseMatrix::operator[] (const size_t index) {
    return ProxyTwo(this, index);
}

ProxyTwoConst SparseMatrix::operator[] (const size_t index) const{
    return ProxyTwoConst(this, index);
}

ProxyTwoConst SparseMatrix::operator* () const{
    return ProxyTwoConst(this, 0);
}

ProxyTwo SparseMatrix::operator* (){
...
SparseMatrix& SparseMatrix::operator=(const SparseMatrix &m){
    ...
    if ((this->size_x != m.size_x) || (this->size_y != m.size_y))
        throw "size mismatch";
    std::size_t num = m.third[size_y];
    first = (double*) malloc(sizeof(double) * num);
    second = (std::size_t*) malloc(sizeof(std::size_t) * num);
    third = new std::size_t[size_y + 1]();
    memcpy(first,m.first,num * sizeof(double));
    memcpy(second,m.second,num * sizeof(std::size_t));
    memcpy(third,m.third,(size_y + 1) * sizeof(std::size_t));
    return *this;
}
...
bool SparseMatrix::operator==(const SparseMatrix &m){
    ...
    if (m.third[size_y] != third[size_y])
        return false;
    for (std::size_t i = 0; i < third[size_y]; ++i)
...
double* SparseMatrix::checkElem(const std::size_t x, const std::size_t y) const{
    if ((x >= size_x) || (y >= size_y))
        throw "bad index";
    if (third == NULL)
        return NULL;
    double *ptr = NULL;
    for (std::size_t i = 0; (i < third[y+1] - third[y]) && (second[third[y] + i] <= x); ++i)
...
void SparseMatrix::set(const std::size_t x, const std::size_t y, double value, int mode){
    if ((x >= size_x) || (y >= size_y))
        throw "bad index";
    if (third == nullptr){
        actual_size = 100;
        ...
        first[0] = value;
        second = (std::size_t*) malloc(sizeof(std::size_t) * 100);
        second[0] = x;
        third = new std::size_t[size_y + 1]();
        for (std::size_t i = y + 1; i < size_y + 1; ++i)
            third[i] = 1;
        return;
    }
    double *val;
    ...
    if (third[size_y]+1 > actual_size){
        ...
        second = (std::size_t*) realloc (second, (third[size_y]+1 + 100) * sizeof(std::size_t));
    }
    ...
    for (std::size_t n = y + 1; n < size_y + 1; ++n)
        third[n] += 1;
}

double SparseMatrix::get(const std::size_t x, const std::size_t y) const {
...
SparseMatrix::SparseMatrix(const SparseMatrix &m) :size_x(m.size_x),size_y(m.size_y) {
    ...
    second = (std::size_t*) malloc ((num+100) * sizeof(std::size_t));
    third = new std::size_t[size_y + 1]();
    memcpy(first,m.first,num * sizeof(double));
    memcpy(second,m.second,num * sizeof(std::size_t));
    memcpy(third,m.third,(size_y + 1) * sizeof(std::size_t));
...
ProxyOne operator+(const size_t i, SparseMatrix &m){
    return ProxyOne(&m,i);
}
...
ProxyOneConst operator+(const size_t i, const SparseMatrix &m){
...
SparseMatrix operator+(const SparseMatrix &m1, const SparseMatrix &m2){
    ...
    m3 = m1;
    std::size_t k = 0;
    for (std::size_t i = 0; i < m2.third[m1.size_y];++i){
...
ProxyTwo ProxyOne::operator*(){
    return ProxyTwo(matr, index1);
}

ProxyOne operator+(const int index, const ProxyOne &p2){
...
ProxyOne& ProxyOne::operator + (const size_t i){
    index1 += i;
    return *this;
}
...
/* ProxyOneConst */
ProxyOneConst operator+(const int index, const ProxyOneConst &p2){
...
ProxyOneConst& ProxyOneConst::operator+ (const size_t i){
    index1 += i;
    return *this;
}
...
ProxyTwoConst ProxyOneConst::operator*(){
    return ProxyTwoConst(matr, index1);
}

/* ProxyTwo */

ProxyTwo operator+(const int index, const ProxyTwo &p2){
...
ProxyTwo& ProxyTwo::operator+ (const size_t i){
    index2 += i;
    return *this;
}
...
double& ProxyTwo::operator[](const int index){
...
/* ProxyTwoConst */
ProxyTwoConst& ProxyTwoConst::operator+ (const size_t i){
    index2 += i;
    return *this;
}
...
ProxyTwoConst operator+(const int index, const ProxyTwoConst &p2){
...
double ProxyTwoConst::operator[](const int index){
    double *ptr = (*matr).checkElem(index1, index2 + index);
    if (ptr == NULL)
        return 0;
